...
app.get(
  ...
  async function (req, res, next) {
    try {
      ...
      const list = await db.output_variable.getPaginated(
...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new outputVariablesAdminAddViewModel(
      db.output_variable,
      "Add output variable",
      "",
      "",
...
app.post(
  ...
  ValidationService.validateInput(
    { name: "required", active_list: "required" },
    {
      "name.required": "Name is required",
      "active_list.required": "ActiveList is required",
    }
  ),
  async function (req, res, next) {
    if (req.session.csrf === undefined) {
    ...
    try {
      ...
      const data = await db.output_variable.insert({
        name,
        active_list: JSON.stringify(active_list),
...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new outputVariablesAdminEditViewModel(
      db.output_variable,
...
app.get(
  ...
  async function (req, res, next) {
    try {
      ...
      var viewModel = new outputVariablesAdminDetailViewModel(
        db.output_variable,
...
app.get(
  "/admin/output-variables-delete/:id",
  SessionService.verifySessionMiddleware(role, "admin"),
  async function (req, res, next) {
    let id = req.params.id;
    ...
    const viewModel = new outputVariablesAdminDeleteViewModel(
      db.output_variable
    );
...
app.get(
  "/admin/termination-variables/:num",
  SessionService.verifySessionMiddleware(role, "admin"),
  async function (req, res, next) {
    try {
      let session = req.session;
      ...
      // viewModel.set_id(req.query.id ? req.query.id : "");
      // viewModel.set_name(
      //   req.query.termination_message ? req.query.termination_message : ""
      // );
      ...
      const count = await db.termination._count(where, []);
      ...
      return res.render("admin/Add_Termination_Variable", viewModel);
    } catch (error) {
      console.error(error);
      viewModel.error = error.message || "Something went wrong";
      return res.render("admin/Add_Termination_Variable", viewModel);
...
app.get(
  "/admin/termination-variables-add",
  SessionService.verifySessionMiddleware(role, "admin"),
  async function (req, res, next) {
    if (req.session.csrf === undefined) {
      req.session.csrf = SessionService.randomString(100);
    ...
    const terminationVariablesAdminAddViewModel = require("../../view_models/termination_variables_admin_add_view_model");

    const viewModel = new terminationVariablesAdminAddViewModel(
      db.termination,
      "Add termination variable",
      "",
      "",
      "/admin/termination-variables-add"
    );
    viewModel.termination = await db.termination.getAll();
    res.render("admin/Add_Termination_variables", viewModel);
  }
);
...
app.post(
  ...
  async function (req, res, next) {
    ...
    const { termination_message, termination_counter } = req.body;
    ...
    try {
      // if (req.validationError) {
      //   viewModel.error = req.validationError;
      //   return res.render("admin/Add_Termination_variables", viewModel);
      // }

      viewModel.session = req.session;

      const terminationVariables = await db.termination.findAll();

      if (terminationVariables.length == 0) {
        const data = await db.termination.insert({
          termination_message: JSON.stringify(termination_message),
          termination_counter: termination_counter,
        });
        ...
        const data = await db.termination.update(
          {
            termination_message: termination_message,
            termination_counter: termination_counter,
          },
          { where: { id: terminationVariables[0].id } }
        );
      ...
      const flashMessageSuccess = req.flash(
        "Termination variable created successfully"
      );
      viewModel.success = flashMessageSuccess[0];
      return res.redirect("/admin/termination-variables-add");
    } catch (error) {
      console.error(error);
      viewModel.error = error.message || "Something went wrong";
      return res.render("admin/Add_Termination_variables", viewModel);
...
//TERMINATION APIS
app.get("/api/v1/termination-variables", async function (req, res, next) {
  try {
    const terminationVariables = await db.termination.findAll();

    const response = { termination: terminationVariables };
